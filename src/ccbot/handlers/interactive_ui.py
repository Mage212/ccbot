...
# Lock to prevent race condition when multiple callers detect UI simultaneously
...
async def handle_interactive_ui(
    ...
    # Use lock to prevent race condition: only one caller can send/edit at a time
